...
class HashUtil:
    ...
    def get_random_prime(self, l, r):
        ...
        while not self.fermat(prime):
            prime = randint(l, r)          # re-draw in the SAME range
        return prime
...
class MyHashSet:
    ...
    def add(self, key: int) -> None:
        ...
        if LL is None or LL.head is None:          # <-- also handle empty-head bucket
            ...
            if node.data == key:
                if node.prev is None:              # head
                    ...
                    if node.nxt is not None:
                        ...
                        self.table[location] = None  # <-- bucket empty, clean up
                else:
                    node.remove_self()
                return
            node = node.nxt
    ...
    def contains(self, key: int) -> bool:
        ...
        if LL is None or LL.head is None:
            return False
        node = LL.head
        while node is not None:                 # check EVERY node incl. tail
            if node.data == key:                # compare data, not the Node
                return True
            node = node.nxt
        return False